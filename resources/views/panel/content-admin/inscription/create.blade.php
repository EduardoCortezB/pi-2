  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Agregar Preinscripción</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="/panel">Inicio</a></li>
              <li class="breadcrumb-item"><a href="{{route('inscriptions')}}">Preinscripción</a></li>
    ...
    <div class="content">
      <div class="container-fluid">
        <form action="{{route('add_inscription')}}" method="POST">
            @csrf
            @method('POST')
        <div class="row">
            <div class="col-md-6">
                <label for="uttn_student">Eres estudiante de la UTT</label>
            ...
            <div class="col-md-12">
                <label for="levelEng">Selecciona el alumno:</label>
                <select class="form-control" name="user_id" id="levelEng">
                    <option selected hidden disabled>Slecciona las opciones</option>
                    @foreach ($students as $studen)
                        <option value="{{$studen->id}}">{{$studen->name}} {{$studen->lastName}}</option>
                    @endforeach
                </select>
                @error('user_id')
                <div class="text-danger">{{ $message }}</div>
